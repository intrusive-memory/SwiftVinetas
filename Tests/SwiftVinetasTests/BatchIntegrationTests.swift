...
struct BatchIntegrationTests {
    ...
    @Test("Generate 4 non-empty images from YNTSWYD Chapter 1 fixture",
    ...
    func generateBatchFromFixture() async throws {
        ...
        try #require(try Vinetas.validateMemory(for: model), "Insufficient memory for \(model.rawValue)")
    ...
    @Test("CLI batch writes 4 non-empty PNG files to output directory",
          .tags(.integration, .gpu),
          .timeLimit(.minutes(10)))
