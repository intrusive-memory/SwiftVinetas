...
struct TrainingResizeTests {
    ...
    func resizedDivisibleBy16() {
        ...
        for size in [33, 47, 63, 99, 255, 1000] {
            ...
            let resized = TrainingDataPreparer.resizeToMultipleOf16(image: image)
            #expect(resized.width % 16 == 0, "Width \(resized.width) not divisible by 16 for input \(size)")
            #expect(resized.height % 16 == 0, "Height \(resized.height) not divisible by 16 for input \(size)")
        }
    }
}
...
struct TrainingPairTests {
    ...
    func trainingPairMatchingBaseNames() {
        ...
        let captionBase = URL(fileURLWithPath: pair.captionPath).deletingPathExtension().lastPathComponent
...
struct TrainingPreparationTests {
    ...
    private func makeTempCharacterDir() throws -> (Character, URL) {
        ...
        try ImageOutput.writePNG(image: testImage, to: referencesDir.appendingPathComponent("front.png"))
        try ImageOutput.writePNG(image: testImage, to: referencesDir.appendingPathComponent("back.png"))

        // Write a source photo
        try ImageOutput.writePNG(image: testImage, to: sourceDir.appendingPathComponent("test-photo-01.png"))
    ...
    func allCaptionsContainTriggerWord() throws {
        ...
        for pair in pairs {
            #expect(pair.caption.contains(character.triggerWord),
