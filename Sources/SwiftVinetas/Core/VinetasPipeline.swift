...
internal enum VinetasPipeline {
  ...
  internal static func generatePanel(
    ...
    // 8. Generate
    log(
      "Generating image: \(style.width)x\(style.height), \(style.steps) steps, seed \(resolvedSeed)"
    )
  ...
  internal typealias StepProgressCallback =
    @Sendable (
      _ currentStep: Int,
      _ totalSteps: Int,
  ...
  internal static func generateSequence(
    ...
    for (index, prompt) in prompts.enumerated() {
      ...
      log(
        "Generating panel \(index + 1)/\(totalPanels): \(style.width)x\(style.height), \(style.steps) steps, seed \(resolvedSeed)"
      )
      ...
      do {
        if useImageToImage {
          result = try await pipeline.generateImageToImageWithResult(
            ...
            onProgress: { currentStep, totalSteps in
              let elapsed =
                Double((clock.now - startTime).components.seconds)
  ...
  internal static func generateFromPromptFile(
    ...
    for index in 0..<totalPanels {
      ...
      log(
        "Generating panel \(index + 1)/\(totalPanels): \(panelStyle.width)x\(panelStyle.height), \(panelStyle.steps) steps, seed \(resolvedSeed)"
      )
